...
var selectable = {
  ...
  transform: function transform(args) {
    ...
    if (args.max > 1) {
      var depth = args.max > 6 ? 3 : args.max > 3 ? 2 : 1;
      checkboxes = [{
        type: "field",
        id: args.id,
        control: 'checkgrid',
  ...
  transform: function transform(args, ctx) {
    ...
    var _loop = function _loop(_lv) {
      ...
      rows.push(_objectSpread(_objectSpread({}, flags), {}, {
        ...
        item: {
          type: "g",
          contents: [].concat(proficiencyItems, iconItems, _toConsumableArray(items), _toConsumableArray(contents))
        }
      }));
    };
...
var row = {
  ...
  render: function render(args, reg, doc) {
    // args.lp = getLabelHeight(args);
    args.rb = getRubyHeight(args);
    var cls = elementClass('row', null, args, ['unlabelled', 'narrow', 'blk', 'pad'], {
      'layout': 'left',
      'valign': 'bottom',
      'rb': '',
      'flex': 'medium'
...
var table = {
  ...
  transform: function transform(args, ctx) {
    ...
    for (var _i5 = 0; _i5 < rows.length; _i5++) {
      if (!isObject(rows[_i5])) {
        warn$1("table", "Row is not an object", rows[_i5]);
      }
...
var field = {
  ...
  render: function render(args, reg, doc) {
    ...
    if (isNull(args.value)) {
      args.value = doc.getVar(args.id, args.typeHint); // if (args.value) log("field", "Value:", args.id, "=", args.value);
    }
    ...
    var cls = elementClass('field', null, args, ["icon", "ref", "misc", "temp", "indent", "blk", "overprint"], {
      "frame": "normal",
      "width": "",
      "align": "centre",
      "size": "medium",
...
function defaultControlRender(args, reg, doc) {
  ...
  var input = "<input".concat(ident.ident).concat(ref).concat(value).concat(attr, ">");
...
var field_control_enum = {
  ...
  render: function render(args, reg, doc) {
    var options = args.options.map(function (opt) {
      ...
      return "<label for='".concat(menuId, "-").concat(slug, "'><input type='radio' name='").concat(menuId, "' value='").concat(slug, "' id='").concat(menuId, "-").concat(slug, "'> ").concat(title, "</label>");
    });
    args.editable = false;
...
var field_control_checkgrid = {
  ...
  defaults: (_defaults = {
    ...
    flex: "tiny",
    depth: 3
  }, _defineProperty(_defaults, "value", 0), _defineProperty(_defaults, "typeHint", 'number'), _defaults),
  render: function render(args) {
    var g = args.group;
...
var Registry = /*#__PURE__*/function () {
  ...
  _createClass(Registry, [{
    ...
    value: function renderItem(item, doc) {
      ...
      if ((doc.largePrint || doc.skipOptional) && has(item, "optional") && item.optional) return '';
...
var Document = /*#__PURE__*/function () {
  ...
  _createClass(Document, [{
    ...
    value: function getCalculations() {
      var fields = [];
      var dependencies = {}; // let references = [];
      ...
      function findCalculationFields(element) {
        function transformCalculation(id, eq) {
          ...
          return "(v) => req([".concat(fields.join(','), "],").concat(eq, ")");
        } // fields
          ...
          if (has(element, "parts")) {
            element.parts.forEach(function (part) {
              if (has(part, "eq") && has(part, "subid")) {
                var id = part.subid == "" ? element.id : element.id + '-' + part.subid;
                fields.push({
                  id: id,
        ...
        if (has(element, "type") && element.type == "table") {
          element.rows.forEach(function (row) {
            return row.cells.forEach(function (elem) {
              return findCalculationFields(elem);
            });
          });
        } // other
      ...
      delete dependencies[0]; // references = [...new Set(references)];
    ...
    value: function getJavascript() {
      ...
      var _this$getCalculations = this.getCalculations(),
        ...
        promise.then(function (data) {
          ...
          ready(function () {
            try {
              ...
              units = system.inferUnits(units); // log("Character", "Inferred units:", units.map(unit => unit.id).sort());
